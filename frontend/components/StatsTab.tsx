    ...
    <div className="w-full flex flex-col items-center px-4 py-4">
      <div className="w-full max-w-2xl flex flex-col gap-8">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="math-card p-4 text-center" style={{ fontFamily: "var(--font-crimson-pro)" }}>
            <div className="text-2xl font-bold" style={{ color: "var(--chalk-blue)" }}>
              {stats.total_routines.toLocaleString()}
            </div>
            <div className="text-xs mt-1" style={{ color: "var(--ink-light)" }}>Routines</div>
          </div>
          <div className="math-card p-4 text-center" style={{ fontFamily: "var(--font-crimson-pro)" }}>
            <div className="text-2xl font-bold" style={{ color: "var(--chalk-purple)" }}>
              {stats.total_files.toLocaleString()}
            </div>
            <div className="text-xs mt-1" style={{ color: "var(--ink-light)" }}>Files</div>
          </div>
          <div className="math-card p-4 text-center" style={{ fontFamily: "var(--font-crimson-pro)" }}>
            <div className="text-2xl font-bold" style={{ color: "var(--chalk-green)" }}>
              {stats.total_loc.toLocaleString()}
            </div>
            <div className="text-xs mt-1" style={{ color: "var(--ink-light)" }}>Lines</div>
          </div>
        </div>

        {/* By routine type */}
        <div className="math-card p-5">
          <h2 className="text-sm font-bold mb-3" style={{ fontFamily: "var(--font-architects-daughter)", color: "var(--ink)" }}>
            By routine type
          </h2>
          <div className="space-y-2">
            {stats.by_routine_type.map((r) => (
              <div key={r.routine_type} className="flex items-center gap-3">
                <span className="w-24 text-xs" style={{ fontFamily: "var(--font-jetbrains-mono)", color: "var(--ink)" }}>
                  {r.routine_type}
                </span>
                <div className="flex-1 h-5 rounded overflow-hidden" style={{ background: "var(--paper-dark)" }}>
                  <div className="h-full rounded" style={{ width: `${(r.count / maxRoutineType) * 100}%`, background: "var(--chalk-blue)" }} />
                </div>
                <span className="w-10 text-right text-xs" style={{ fontFamily: "var(--font-jetbrains-mono)", color: "var(--ink-light)" }}>
                  {r.count}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {stats.by_precision.map((r) => (
              <div key={r.precision_type} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-5 rounded overflow-hidden" style={{ background: "var(--paper-dark)" }}>
                  <div className="h-full rounded" style={{ width: `${(r.count / maxPrecision) * 100}%`, background: "var(--chalk-purple)" }} />
                </div>
                <span className="w-10 text-right text-xs" style={{ fontFamily: "var(--font-jetbrains-mono)", color: "var(--ink-light)" }}>
                  {r.count}
                </span>
          ...
          <div className="space-y-2">
            {stats.largest_routines.map((r) => (
              <div key={r.subroutine_name} className="flex items-center gap-3">
                ...
                <span className="w-12 text-right text-xs" style={{ fontFamily: "var(--font-jetbrains-mono)", color: "var(--ink-light)" }}>
                  {r.lines}
                </span>
              </div>
            ))}
